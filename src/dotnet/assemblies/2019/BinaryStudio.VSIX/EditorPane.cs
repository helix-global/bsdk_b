...
using BinaryStudio.VSShellPackage.Models;
...
namespace BinaryStudio.VSShellPackage
    {
    public class EditorPane : WindowPane,
            IOleCommandTarget,
            IVsPersistDocData,
            IPersistFileFormat,
            IVsToolboxUser
        ...
        private Int32 FormatIndex { get;set; }
        ...
        Int32 IPersist.GetClassID(out Guid ClassId)
            {
            ClassId = typeof(EditorFactory).GUID;
            return VSConstants.S_OK;
            }
        #endregion
        ...
        /// <returns>If the method succeeds, it returns <see cref="F:Microsoft.VisualStudio.VSConstants.S_OK"/>. If it fails, it returns an error code.</returns>
        Int32 IPersistFileFormat.Load(String filename, UInt32 Mode, Int32 ReadOnly)
            {
            FileName = filename;
            DataContext = VSPackage.MetadataScope.Load(filename);
            var ModelContext = (Object)DataContext;
            ...
            ElementHostControl.DataContext = ModelContext;
            return VSConstants.S_OK;
            }
        #endregion
        ...
        Int32 IPersistFileFormat.GetFormatList(out String FormatList)
            {
            FormatList = "Executable Files(*.exe;*.dll;*.ocx;*.sys)\n*.exe;*.dll;*.sys";
            return VSConstants.S_OK;
            }
        #endregion
        ...
        private void PrivateInit()
            {
            ElementHostControl = new OverviewPaneHostControl();
        ...
        private OverviewPaneHostControl ElementHostControl;
